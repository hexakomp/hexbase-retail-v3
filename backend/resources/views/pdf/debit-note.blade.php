...
<head>
    <meta charset="UTF-8">
    <title>Debit Note — {{ $note->debit_note_number }}</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .doc-title { font-size: 13pt; font-weight: bold; text-align: right; color: #e65100; }
        ...
        .lines-table th { background: #e65100; color: #fff; padding: 5px 4px; font-size: 8pt; text-align: center; border: 1px solid #e65100; }
        .lines-table td { border: 1px solid #bdbdbd; padding: 4px; font-size: 8.5pt; vertical-align: top; }
        .lines-table tr:nth-child(even) td { background: #fff3e0; }
        ...
        .totals-table .grand-total td { font-weight: bold; font-size: 10pt; border-top: 2px solid #e65100; background: #fff3e0; }
        ...
        <tr>
            ...
            <td style="text-align:right; width:40%;">
                ...
                <div style="font-size:9pt; margin-top:4px;">
                    <strong>No:</strong> {{ $note->debit_note_number }}<br>
                    <strong>Date:</strong> {{ $note->debit_note_date->format('d/m/Y') }}
                </div>
                <div style="margin-top:6px; color:#e65100; font-size:8pt; font-weight:bold;">
        ...
        <tr>
            <td>
                <div class="info-box">
                    ...
                    <div class="info-value" style="font-weight:bold;">{{ $note->vendor->name }}</div>
                    @if($note->vendor->gstin)
                        <div style="font-size:8pt; margin-top:2px;">GSTIN: {{ $note->vendor->gstin }}</div>
                    @endif
                    @if($note->vendor->address)
                        <div style="font-size:8pt; margin-top:2px;">{{ $note->vendor->address }}</div>
                    @endif
                </div>
            </td>
            <td>
                <div class="info-box">
                    @if($note->purchaseInvoice)
                        <div class="info-label">Against Purchase Invoice</div>
                        <div class="info-value">{{ $note->purchaseInvoice->invoice_number }}</div>
                        <div style="font-size:8pt;">Dated: {{ $note->purchaseInvoice->invoice_date->format('d/m/Y') }}</div>
                    @endif
                    @if($note->reason)
        ...
        <tr>
            <td style="width:60%">
                <div style="font-size:8pt; color:#555;">
                    This debit note is computer-generated and does not require a signature.
                </div>
            </td>
