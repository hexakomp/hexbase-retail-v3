...
class CreditNoteFormScreen extends ConsumerStatefulWidget {
  final int? noteId;
  const CreditNoteFormScreen({super.key, this.noteId});
...
class _CreditNoteFormScreenState extends ConsumerState<CreditNoteFormScreen> {
  ...
  Future<void> _loadNote() async {
    ...
    try {
      ...
      setState(() {
        _noteData = note;
        _dateController.text = (note['credit_note_date'] as String).substring(
          0,
          10,
        );
        _reasonController.text = note['reason'] as String? ?? '';
        _customerId = note['customer_id'] as int;
        _salesInvoiceId = note['sales_invoice_id'] as int?;
        _loading = false;
      });
    } catch (e) {
      setState(() {
  ...
  Future<void> _save() async {
    ...
    try {
      final repo = ref.read(creditNoteRepositoryProvider);
      final payload = {
        'credit_note_date': _dateController.text,
  ...
  Future<void> _cancelNote() async {
    ...
    try {
      final repo = ref.read(creditNoteRepositoryProvider);
      await repo.cancel(widget.noteId!);
      if (mounted) Navigator.pop(context);
    } catch (e) {
      setState(() {
        _error = e.toString();
  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(
      title: _isEdit ? 'Credit Note' : 'New Credit Note',
